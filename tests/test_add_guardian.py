...
def setup_env_vars():
    os.environ["OrbDex_ACCOUNT_ADDRESS"] = "0x123"
    os.environ["OrbDex_ACCOUNT_KEY"] = "0x456"
    yield
    os.environ.pop("OrbDex_ACCOUNT_ADDRESS")
    os.environ.pop("OrbDex_ACCOUNT_KEY")
...
async def test_load_contract_from_account(mock_account):
    ...
    with patch(
        "OrbDex_cli.main.Contract.from_address",
...
async def test_change_guardian(mock_account, setup_env_vars):
    ...
    mock_pub_key = "0x789"
    with (
        patch("OrbDex_cli.main.load_contract_from_account", return_value=mock_contract),
        patch("OrbDex_cli.main._process_invoke", new_callable=AsyncMock),
    ):
...
async def test_sign_invoke_tx(mock_account, setup_env_vars):
    ...
    with (
        patch("OrbDex_cli.main.load_invoke", return_value=mock_invoke),
        patch("OrbDex_cli.main.print_invoke", return_value=None),
...
async def test_submit_invoke_tx(mock_account, setup_env_vars):
    ...
    with (
        patch("OrbDex_cli.main.load_invoke", return_value=mock_invoke),
        patch(
            "OrbDex_cli.main.load_signature", side_effect=lambda f: mock_signature.get(f.name, [])
        ),
        patch("OrbDex_cli.main._fetch_signers_pubkeys", return_value=["0x1", "0x2", "0x3"]),
        patch("OrbDex_cli.main.load_contract_from_account", return_value=MagicMock()),
        patch("builtins.open", mock_open(read_data="data")),
    ):
...
async def test_escape_guardian_logic():
    ...
    with patch("OrbDex_cli.main._check_multisig_required", return_value=False), \
         patch("OrbDex_cli.main._process_invoke", new_callable=AsyncMock) as mock_process:
...
def test_print_account_info_command(setup_env_vars):
    with (
        patch("OrbDex_cli.main.OrbDexAccount") as mock_account,
        patch("OrbDex_cli.main.asyncio.run") as mock_async_run,
        patch("OrbDex_cli.main.load_contract_from_account", new_callable=AsyncMock),
    ):
...
def test_add_guardian_backup_command(setup_env_vars):
    ...
    with (
        patch("OrbDex_cli.main.OrbDexAccount", return_value=mock_account_obj) as mock_account,
        patch("OrbDex_cli.main.asyncio.run", side_effect=mock_asyncio_run) as mock_async_run,
        patch("OrbDex_cli.main.load_contract_from_account", new_callable=AsyncMock, return_value=mock_contract),
        patch("OrbDex_cli.main._check_multisig_required", new_callable=AsyncMock, return_value=False),
        patch("OrbDex_cli.main._process_invoke", new_callable=AsyncMock),
        patch("OrbDex_cli.main.KeyPair.from_private_key", return_value=MagicMock()),
...
def test_sign_invoke_tx_command(setup_env_vars):
    ...
    with (
        ...
        patch("OrbDex_cli.main.asyncio.run", side_effect=mock_asyncio_run) as mock_async_run,
        patch("OrbDex_cli.main.load_invoke", return_value=mock_invoke),
        patch("OrbDex_cli.main.print_invoke"),
        patch("builtins.open", new_callable=mock_open),
    ):
...
def test_submit_invoke_tx_command(setup_env_vars):
    ...
    with (
        ...
        patch("OrbDex_cli.main.asyncio.run", side_effect=mock_asyncio_run) as mock_async_run,
        patch("OrbDex_cli.main.load_invoke", return_value=mock_invoke),
        patch("OrbDex_cli.main.load_signature", return_value={"0x1": [1, 2, 3]}),
        patch("OrbDex_cli.main.load_contract_from_account", new_callable=AsyncMock, return_value=mock_contract),
        patch("OrbDex_cli.main._fetch_signers_pubkeys", new_callable=AsyncMock, return_value=["0x1", "0x2", "0x3"]),
        patch("builtins.open", new_callable=mock_open),
    ):
