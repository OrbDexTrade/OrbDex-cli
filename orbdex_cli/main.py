...
import secrets
import typer
from OrbDex_py.account.starknet import Account as StarknetAccount
from OrbDex_py.OrbDex import OrbDex, OrbDexAccount
...
def check_env_vars():
    ...
    """
    required_vars = ["OrbDex_ACCOUNT_ADDRESS", "OrbDex_ACCOUNT_KEY"]
...
ACCOUNT_ADDRESS = os.environ.get("OrbDex_ACCOUNT_ADDRESS")
ACCOUNT_KEY = os.environ.get("OrbDex_ACCOUNT_KEY")
...
async def _sign_invoke_tx(paccount: OrbDexAccount, file_path: str):
...
async def _submit_invoke_tx(paccount: OrbDexAccount, tx_file, sig_files):
...
def print_account_info(
    ...
    Returns:
        None
    """
    pclient = OrbDex(env=env)
    paccount = OrbDexAccount(config=pclient.config, l1_address="0x0", l2_private_key=ACCOUNT_KEY)
...
async def _withdraw_to_l1(paccount: OrbDexAccount, l1_recipient: str, amount_decimal: Decimal):
...
async def _transfer_on_l2(
    paccount: OrbDexAccount, target_l2_address: str, amount_decimal: Decimal
):
    paraclear_address = paccount.config.paraclear_address
    usdc_address = paccount.config.bridged_tokens[0].l2_token_address
...
async def _deposit_to_paraclear(paccount: OrbDexAccount, amount_decimal: Decimal):
...
async def _trigger_escape_guardian(paccount: OrbDexAccount):
    contract = await load_contract_from_account(paccount.l2_address, paccount)
